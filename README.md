...
新增可移植安装器、素材自动采集、完整中文字体和可运行的开头/正文工程。Mac 相册、人物右侧动效、时间线与正文工作台按内容选用；同时保留每期开场变化与反 PPT 验收。

**[下载最新安装包](https://github.com/3105059695-rgb/ui-motion-explainer/releases/latest)** · **[复制给朋友的提示词](PROMPT.md)** · **[安装与首次验证](START-HERE.md)** · **[4 种可运行示例](assets/portable-studio/README.md)**

需要具备文件/终端能力的 AI 助手与 Python 3.10+；首次自动安装渲染依赖和浏览器。无需作者私有账号、路径、NVIDIA 显卡或 AE 软件。环境准备好后，代理负责找素材和按口播设计，不只是替换模板文字。包内没有任何人的肖像、音色和密钥；[实际验证范围](docs/portable-verification.md)。
